...
def draw_tree(tree, brecon, stree,
              ...
              rmargin=10, lmargin=30, tmargin=0, bmargin=0,
              tree_color=(0, 0, 0),
              tree_trans_color=(1, 0, 0),
              stree_color=(.4, .4, 1),
              snode_color=(.2, .2, .7),
              dup_color=(0, 1, 0),
              dup_color_border=(.5, .5, 0),
              trans_color=(1, 0, 0),
              trans_color_border=(1, 1, 1),
              event_size=10,
              snames=None,
              rootlen=120,
              stree_width=.5,           #default .8
              filename="tree.svg"
              ):
    ...
    if rootlen is None:
        ...
        #print .1 * max(l[0] for l in slayout.values())
    # setup slayout
    ...
    for node in tree.preorder():
        ...
        if event == "spec" or event == "gene" or event == "specloss":
    ...
    for node in tree.postorder():
        snode, event = brecon[node][-1]
        if event != "spec" and event != "gene" and event != "specloss":
            v = [yorders[child]
